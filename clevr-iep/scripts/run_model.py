...
def main(args):
  ...
  if args.question is not None and args.image is not None:
    ...
    with open('val_log.txt','w') as f:
    	for i in range(500):
    		print(i)
    		args.question=data['questions'][i]['question']
    		args.image='data/CLEVR_v1.0/images/val/'+data['questions'][i]['image_filename']
    		pred = run_single_example(args,model)
    		if pred == data['questions'][i]['answer']:
    			print("Correct")
    		else:
    			f.write(data['questions'][i]['question'])
    			f.write(' ')
    			f.write(data['questions'][i]['image_filename'])
    			f.write(' ')
    			f.write(data['questions'][i]['answer'])
    			f.write(' ')
    			f.write(pred)
    			f.write('\n')
    f.close()
...
def run_single_example(args, model):
  ...
  #print("pred_ans_idx:",predicted_answer_idx)
  #predicted_answer_idx = predicted_answer_idx.item()
