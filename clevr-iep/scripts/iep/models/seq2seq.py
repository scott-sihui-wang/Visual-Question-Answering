...
class Seq2Seq(nn.Module):
  ...
  def reinforce_sample(self, x, max_length=30, temperature=1.0, argmax=False):
    ...
    #print('cur_input',cur_input,'shape:',cur_input.size())
    h, c = None, None
    ...
    for t in range(T):
      ...
      #print("logprobs:",logprobs,"shape:",logprobs.size())
      logprobs=logprobs.view(N,-1)
      #print("logprobs:",logprobs,"shape:",logprobs.size())
      probs = F.softmax(logprobs) # Now N x V
      #print("probs:",probs,"shape:",probs.size())
      if argmax:
        _, cur_output = probs.max(1)
        #print("Case A")
        #print("cur_output:",cur_output,"shape:",cur_output.size())
        
        #if(len(cur_output.size())==1):
      ...
      if done.sum() == N:
        #print("break")
        break
      #print(Variable(y.type_as(x.data)))
    #print(x)
    #print(x.data)
    #print(type(x.data))
    #print(Variable(y.type_as(x.data)))
